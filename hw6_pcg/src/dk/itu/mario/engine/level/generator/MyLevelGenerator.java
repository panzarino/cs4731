...
public class MyLevelGenerator{
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		char[] chrome = new char[195];

		for (int i = 0; i < chrome.length; i++) {
			chrome[i] = (char)('a' + rand.nextInt(26));
		}

		individual.setChromosome(String.valueOf(chrome));
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		decision = getBestIndividual(population).getFitness() > 0.8 || count > 2500;
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		int maxIndex = rand.nextInt(population.size());
	...
	private MyDNA pickIndividualForCrossover (ArrayList<MyDNA> population, MyDNA excludeMe)
	{
		...
		while (picked == null || picked == excludeMe) {
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		...
		ArrayList<MyDNA> fullPopulation = new ArrayList<MyDNA>();

		fullPopulation.addAll(oldPopulation);
		fullPopulation.addAll(newPopulation);

		for (int i = 0; i < getPopulationSize(); i++) {
			MyDNA fittest = getBestIndividual(fullPopulation);
			fullPopulation.remove(fittest);
